...
/*   Updated: 2026/04/09 20:03:20 by andriraz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_format(char c, va_list arg, int fd)
{
	...
	if (c == 'p')
		return (ft_conv((unsigned long)va_arg(arg, void *), "0123456789abcdef",
				fd));
	if (c == 'd')
	...
	if (c == 'x')
		return (ft_convert_base(va_arg(arg, unsigned int), "0123456789abcdef",
				fd));
	if (c == 'X')
		return (ft_convert_base(va_arg(arg, unsigned int), "0123456789ABCDEF",
				fd));
